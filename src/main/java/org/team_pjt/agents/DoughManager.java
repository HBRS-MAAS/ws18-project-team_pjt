...
import org.team_pjt.Objects.CurrentProcessedProduct;
...
public class DoughManager extends BaseAgent {
    ...
    private Vector<CurrentProcessedProduct> vProductsToKnead;
    ...
    private Vector<CurrentProcessedProduct> vAlreadyKneadedOrders;
    private Vector<CurrentProcessedProduct> vAlreadyPreparedOrders;
    private Vector<CurrentProcessedProduct> vProductsToPrepare;
...
//                    }
        ...
        private void checkWhetherKneadingSizeIsEqual() {
            ...
            for (OrderDoughPrep odpKneaded : vOrder) {
                if (odpKneaded.getDeliveryDate() == getCurrentDay()) {
                    Iterator<CurrentProcessedProduct> iProductsToKnead = vProductsToKnead.iterator();
                    while (iProductsToKnead.hasNext()) {
                        CurrentProcessedProduct ckpNext = iProductsToKnead.next();
                    ...
                    if (!bOrderAlreadyExists) {
                        ...
                        while (iBakedGoods.hasNext()) {
                            ...
                            vProductsToKnead.add(new CurrentProcessedProduct(odpKneaded.getGuid(), bgNext.getName(), bgNext.getAmount(), odpKneaded.getDeliveryDate()));
                        }
                    }
                }
            }
        ...
        private void calculatePreaparationTime(Vector<CurrentProcessedProduct> bakedGoods) {
            Iterator<CurrentProcessedProduct> iBakedGoods = bakedGoods.iterator();
            while (iBakedGoods.hasNext()) {
                CurrentProcessedProduct bgNext = iBakedGoods.next();
                    ...
                    while (iDoughPrepTables.hasNext()) {
                        ...
                        if (!(dptNext.isbBusy())) {
                            if (dptNext.getsCurrentKneadedProduct() != "") {
                                if (!isbProductOfOrderAlreadyKneaded(false, bgNext, vAlreadyPreparedOrders)) {
                                    vAlreadyPreparedOrders.add(new CurrentProcessedProduct(dptNext.getsCurrentKneadedOrder(), dptNext.getsCurrentKneadedProduct(), dptNext.getiAmount(), -1));
                                    jsoPreparedItems = new JSONObject();
        ...
        private void calculateKneadingTime(Vector<CurrentProcessedProduct> bakedGoods) {
            Iterator<CurrentProcessedProduct> iBakedGoods = bakedGoods.iterator();
            while (iBakedGoods.hasNext()) {
                CurrentProcessedProduct bgNext = iBakedGoods.next();
                HashMap<String, Integer> hmDuration = hmRecipeProducts.get(bgNext.getsProduct());
                    ...
                    while (iKneadingMachines.hasNext()) {
                        ...
                        if (!(kmNext.isbBusy())) {
                            if (kmNext.getsCurrentKneadedProduct() != "") {
                                if (!isbProductOfOrderAlreadyKneaded(false, bgNext, vAlreadyKneadedOrders)) {
                                    vAlreadyKneadedOrders.add(new CurrentProcessedProduct(kmNext.getsCurrentKneadedOrder(), kmNext.getsCurrentKneadedProduct(), kmNext.getiAmount(), -1));
                                    vProductsToPrepare.add(new CurrentProcessedProduct(kmNext.getsCurrentKneadedOrder(), kmNext.getsCurrentKneadedProduct(), kmNext.getiAmount(), -1));
        ...
        private void checkWhetherAlreadyPreparing(Integer iAmount, String sPreparingItem, CurrentProcessedProduct bgNext, HashMap<String, Integer> hmDuration, KneadingPreparingMachine kmNext, Vector<KneadingPreparingMachine> vKneadingPreparingMachine) {
        ...
        private boolean isbProductOfOrderAlreadyKneaded(boolean bProductOfOrderAlreadyPrepared, CurrentProcessedProduct bgNext, Vector<CurrentProcessedProduct> vAlreadyKneadedOrders) {
            Iterator<CurrentProcessedProduct> iVAlreadyPreparedOrders = vAlreadyKneadedOrders.iterator();
            while (iVAlreadyPreparedOrders.hasNext()) {
                CurrentProcessedProduct ckpNext = iVAlreadyPreparedOrders.next();
