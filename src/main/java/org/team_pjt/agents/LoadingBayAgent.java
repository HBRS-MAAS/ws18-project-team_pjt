...
public class LoadingBayAgent extends Agent {
    ...
    protected void setup() {
        ...
        if (!readArgs(args)) {
            System.out.println("not enough arguments for LoadingBayAgent!");
        }
        addBehaviour(new loadingDoneMessage());
    ...
    private class loadingDoneMessage extends CyclicBehaviour {

        @Override
        public void action() {
            MessageTemplate mt = MessageTemplate.and(MessageTemplate.MatchPerformative(ACLMessage.INFORM),
                    MessageTemplate.and(MessageTemplate.MatchConversationId("loading-done"), MessageTemplate.MatchSender(transmitter_agent)));
            ACLMessage msg = myAgent.receive(mt);
            if (msg != null) {
                ACLMessage reply = new ACLMessage(ACLMessage.INFORM);
                reply.setConversationId("loading-done");
