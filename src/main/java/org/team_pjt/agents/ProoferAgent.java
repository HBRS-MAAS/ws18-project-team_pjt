...
public class ProoferAgent extends Agent {
    ...
    private class doughPrepDoneMessage extends CyclicBehaviour {

        @Override
        public void action() {
            MessageTemplate mt = MessageTemplate.and(MessageTemplate.MatchPerformative(ACLMessage.INFORM),
                    MessageTemplate.and(MessageTemplate.MatchConversationId("dough-prep-done"), MessageTemplate.MatchSender(transmitter_agent)));
            ACLMessage msg = myAgent.receive(mt);
            if (msg != null) {
                ACLMessage reply = new ACLMessage(ACLMessage.INFORM);
                reply.setConversationId("dough-prep-done");
